...
def sql_select_single_teacher_courses(db_cursor, user_email: str):
    db_cursor.execute(
...
def sql_delete_user(db_cursor, user_email: str):
    db_cursor.execute("DELETE FROM users WHERE email = %s", (user_email,))


def sql_give_admin_permissions(db_cursor, user_email: str):
